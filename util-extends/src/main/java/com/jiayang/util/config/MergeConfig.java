package com.jiayang.util.config;
...
public class MergeConfig {
    ...
    private void mergeListDistinct(List target,List append) {
    	target.removeAll(append);
    	target.addAll(append);
	...
	private void mergeMap(Map<String,Object> targetMap,Map<String,Object> appendMap ) {
		...
		{
			if(!targetMap.containsKey(key)&&!(appendMap.get(key) instanceof Map)&&!(appendMap.get(key) instanceof List))
			{
			...
			else if((appendMap.get(key) instanceof List)&&!(appendMap.get(key).equals(targetMap.get(key))))
			{
				if(targetMap.get(key)==null)
				targetMap.put(key,new ArrayList<>());
				mergeListDistinct((List)targetMap.get(key),(List)appendMap.get(key));
			}
		}
